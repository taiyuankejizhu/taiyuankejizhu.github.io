$(document).ready(function() {
    ...
    function selectCam() {
        var sel = document.getElementById('videosrc');
        sel.selectedIndex = 0;            
    }
    ...
    function setWebcamAndMic(audio_source, video_source) {
        ...
        await navigator.mediaDevices.getUserMedia({ audio: {optional: [{sourceId: audio_source}]},
    ...
    //peer  = new Peer(u_id+'_cam',{host: 'www.bunver.com', port: 9000, path: '/peerjs', secure:true});
	peer = new Peer(u_id+'_cam', {debug: 2});
	
    // peer id assignment
    peer.on('open', function(id) {
        $('#log').val($('#log').val()+'\n'+'Peer ID: ' + id);
        console.log('Peer Cam ID: ' + id);
    });
